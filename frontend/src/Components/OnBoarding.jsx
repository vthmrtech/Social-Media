...
const OnBoarding = () => {
    ...
    const [onboarding, setonboarding] = useState({ username: "", mobile: "", dob: "", bio: "" })
    const profileData = async (e) => {
        if (e.target.value != "") {
            if (e.target.type === "file") {
                if (e.target.files[0]?.type.includes("image")) {

                    onboarding[e.target.name] = await toBase64(e.target.files?.[0])
                ...
                else if ((new Date().getFullYear() - new Date(e.target.value).getFullYear()) < 13) {
    ...
    const addProfile = (e) => {
        ...
        for (let x in onboarding) {
            if ((onboarding[x] == undefined || onboarding[x] == "")) {
                error[x] = "Required*"
            }
    ...
    return (
        <>
            <div className='onboardingPage'>
                <Box sx={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: "800px", background: "#645a5a8f", padding: 3 }}>
                    <Form onSubmit={addProfile} className="signUp p-3  onboardingForm">
                        <legend className='fw-bold text-white text-center fs-1'> Profile Info</legend>
                        <Box sx={{ display: "flex", color: "white" }}>
                            <div className='w-100'>
                                <div className='w-100 d-flex flex-column align-items-center' >
                                    ...
                                    <Form.Control type="file" name="profileImg" onChange={profileData} className='d-none' id='profile' />
                                    {
                                        onboarding['profileImg'] && onboarding['profileImg'] != profile
                                            ?
                            ...
                            <div className='w-100'>
                                ...
                                <Form.Control type="text" name="username" onChange={profileData} />
                                ...
                                <Form.Control type="text" name="mobile" onChange={profileData} />
                                ...
                                <Form.Control as="textarea" name='bio' rows={3} onChange={profileData} />
